...
// project headers
#include "coregl/color.h"
#include "coregl/pixel_format.h"
...
public:
    ...
    opengl_context(const coregl::hdc_wrap& p_hdc, opengl_context& p_reference);

    // Initialize a legacy opengl context
    // Used to initialize function pointers for a real context
    struct t_legacy_ctor_tag {};
    opengl_context(const coregl::hdc_wrap& p_hdc, t_legacy_ctor_tag);
    ...
    int assign_pixel_format(const t_pixel_format& p_format);
    ...
    void clear_frame(const color<float>& p_color);
...
private:
    ...
    std::vector<int> make_pixel_attributs(const t_pixel_format& p_format);
